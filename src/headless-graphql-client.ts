...
export class HeadlessGraphQLClient implements IHeadlessGraphQLClient {
  ...
  async getGarageUnloadEvents (blockIndex: number, agentAddress: Address, avatarAddress: Address): Promise<GarageUnloadEvent[]> {
    ...
    return data.data.transaction.ncTransactions.map(tx => {
      ...
      const values = (action.get('values') as Dictionary).get('l') as any[] // FIXME: Correct type.
      const recipientAvatarAddress = Address.fromBytes(values[0])
      const fungibleAssetValues: Array<[Address, FungibleAssetValue]> =
                (values[1] ?? []).map(args => [
      ...
      const fungibleItems: Array<[Address, string, number]> =
                (values[2] ?? []).map(args => [
  ...
  async getAssetTransferredEvents (blockIndex: number, recipient: Address): Promise<AssetTransferredEvent[]> {
  ...
  private async graphqlRequest (
    ...
    try {
      ...
      if (response.data.errors !== undefined) throw response.data.errors
