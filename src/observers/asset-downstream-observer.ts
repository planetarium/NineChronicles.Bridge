...
export class AssetDownstreamObserver implements IObserver<{
  blockHash: BlockHash
  events: Array<AssetTransferredEvent & TransactionLocation>
}> {
  ...
  async notify (data: { blockHash: BlockHash, events: Array<AssetTransferredEvent & TransactionLocation> }): Promise<void> {
    const { events } = data
